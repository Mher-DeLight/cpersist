...
bool SaveManager::change_file(const std::string& new_file) {
    if (!filename_fits_standards(new_file)) {
        return false;
...
uint64_t SaveManager::getDataPosition(const std::string& name, const bool loose) {
    ...
    while (position < data.size()) {
        ...
        // the position now points at the data itself. move back to dataSize then return it if
        // there's a match.
...
void SaveManager::commit() {
    if (current_file.empty()) {
        cpersist_internal::ErrorManager::get().throwError(
            "Can't commit changes while no file is open.");
    }
    std::ofstream file(
        fullFilePath,
        std::ios::binary |
